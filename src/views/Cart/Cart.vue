  ...
  <div class="chat">
    <div v-if="userinfo.id">
      ...
      <main class="jd_shopCart_list">
        <section>
          <div class="shopCart_list_con" v-for="(goods,index) in usercartgoods" :key="index">
            <div class="list_con_left">
              <a
                href="javascript:;"
                class="cart_check_box"
                :checked="goods.checked"
                @click="singleSelectedGood(index)"
              ></a>
            </div>
            <div class="list_con_right">
              ...
              <div class="shop_con">
                ...
                <p class="shop_price">&yen;{{goods.price}}</p>
                <div class="shop_deal">
                  <div class="shop_deal_left">
                    <span @click.prevent="updateGoodsCount(index,false)">-</span>
                    <input type="tel" value="1" v-model="goods.buy_count" disabled />
                    <span @click.prevent="updateGoodsCount(index,true)">+</span>
                  </div>
                  <div class="shop_deal_right" @click="deleteCartGood(index)">
                    <span></span>
                    <span></span>
                  </div>
                </div>
...
export default {
  ...
  computed: {
    ...mapState(['userinfo', 'usercartgoods']),
  },
  mounted() {
    this.$store.dispatch('reqUserCartGoods')
  },
  components: {
    SelectLogin,
  },
  methods: {
    updateGoodsCount(index, isAdd) {
      this.$store.dispatch('updateGoodsCount', { index, isAdd })
      this.getAllPrice()
    },
    selectedAll() {
    ...
    singleSelectedGood(index) {
      this.$store.dispatch('singleSelectedGood', { index })
      this.hasSelectedAll()
      this.getAllPrice()
    },
    getAllPrice() {
      //计算总价格
      let totalPrice = 0
      this.usercartgoods.forEach((goods, index) => {
        if (goods.checked) {
    ...
    deleteCartGood(index) {
      //删除购物车
      MessageBox.confirm('您删除该商品？').then((action) => {
        if (action === 'confirm') {
          this.$store.dispatch('deleSingleSelectedGood', { index })
          this.getAllPrice()
        }
      })
    },
